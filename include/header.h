...
/* 64-bit diagnostic warnings.
   When INTG_IS_LONG is not defined, intg is 32-bit int.
   These macros warn at runtime when a value would overflow
   the 32-bit range, helping diagnose 64-bit cleanliness issues.
   When INTG_IS_LONG is defined, the checks are no-ops. */
...
#define INTG_CHECK_OVERFLOW(val, context) do { \
    long _intg_v = (long)(val); \
    if (_intg_v > INT_MAX || _intg_v < INT_MIN) \
        fprintf(stderr, "LUSH 64-bit WARNING: intg overflow in %s " \
                "(value=%ld, INT_MAX=%d)\n", (context), _intg_v, INT_MAX); \
} while(0)

#define SIZE_CHECK_OVERFLOW(val, context) do { \
    long _size_v = (long)(val); \
    if (_size_v > INT_MAX || _size_v < 0) \
        fprintf(stderr, "LUSH 64-bit WARNING: size overflow in %s " \
                "(value=%ld, INT_MAX=%d)\n", (context), _size_v, INT_MAX); \
} while(0)

#else

#define INTG_CHECK_OVERFLOW(val, context) ((void)0)
#define SIZE_CHECK_OVERFLOW(val, context) ((void)0)
